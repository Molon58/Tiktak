...
namespace AspNetCoreTiktakProject.App.Service.Services
{
    public class CarSaleService : ICarSaleService
    {
        ...
        public async Task SaleSave(int id, string userName)
        {
            ...
            CarProperty cp = _uow.GetRepository<CarProperty>().GetSync(cs => cs.Id == id);
        ...
        public async Task EndDrive(BillViewModel? model, string userName )
        {
           ...
           int carSaleId= carSales.Max(x => x.Id);
            ...
            CarProperty car = _uow.GetRepository<CarProperty>().GetSync(c => c.Id==model.CarSaleId);
            CarProperty cp = _uow.GetRepository<CarProperty>().GetSync(cp => cp.Id==model.CarSaleId); //carsaleıd aslında carıd-----cp.ıd
