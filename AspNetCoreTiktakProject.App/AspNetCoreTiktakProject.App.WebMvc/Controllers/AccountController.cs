...
namespace AspNetCoreTiktakProject.App.WebMvc.Controllers
{
    public class AccountController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Register(RegisterViewModel model, IFormFile formFile)      //Kullanıcının kaydını oluşturmak ve kayıt işleminin sonucuna göre yönlendirme veya hata mesajı eklemektir.
        {
            var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot\\images", formFile.FileName);
            var stream = new FileStream(path, FileMode.Create);
            formFile.CopyTo(stream);
            model.DriverLicence = "/images/" + formFile.FileName /*+ model.Id*/;    //Yüklenen resim isimlerinde çakışma olmaması için ismin sonuna uniq id bilgisini ekliyoruz
